...
export default function DeepDives({ posts = [] }: { posts?: SanityPost[] }) {
  ...
  // 🔥 ADIÓS AL CÓDIGO INÚTIL DE RESIZE DE LA VENTANA 🔥
  ...
  useGSAP(() => {
    ...
    // 🔥 OPTIMIZACIÓN GSAP: Sustituimos el fromTo pesado por un to ligero de opacidad. 
    // Y le dejamos el parallax pesado al CSS.
    const reveals = gsap.utils.toArray<HTMLElement>('.dd-reveal');
    gsap.set(reveals, { y: 20, opacity: 0 });

    ScrollTrigger.batch(reveals, {
      start: "top 90%", // Un poco más de margen
      once: true,
      onEnter: (elements) => {
        gsap.to(elements, {
          y: 0, 
    ...
    <section ref={container} className="bg-pangea-neutral font-body w-full overflow-hidden pt-16 md:pt-24 pb-16 md:pb-20 transition-colors duration-500">
      ...
      <div className="flex w-full max-w-[1600px] justify-center items-start gap-6 lg:gap-12 px-4 sm:px-6 mx-auto relative z-10">
        ...
        <div className="max-w-[1200px] w-full flex flex-col gap-8 md:gap-10">
          ...
          <div className="immersion-zone relative w-full min-h-[500px] md:min-h-[600px] rounded-3xl md:rounded-[2rem] overflow-hidden border border-white/10 shadow-[0_0_80px_rgba(0,0,0,0.5)] bg-pangea-card/20 group">
            
            {/* 🌌 CAPA DE FONDO (AHORA PARALLAX POR CSS) */}
            <div className="absolute inset-0 z-0 pointer-events-none overflow-hidden">
              {bgToUse && (
                <div 
                  /* 🔥 MAGIA DE TAILWIND: Le damos un scale sutil que crece al hacer hover a toda la tarjeta grande en lugar de pelear con el scroll */
                  className="absolute inset-0 w-full h-full bg-cover bg-center blur-[2px] opacity-100 md:opacity-75 transition-transform duration-[2s] ease-out group-hover:scale-105"
              ...
              >
                 ...
                 <div className="relative z-10 flex flex-col md:flex-row items-start md:items-center gap-6 md:gap-10 w-full">
                    ...
                    <div className="flex-grow">
                      ...
                      <h3 className="font-headline text-white text-2xl sm:text-3xl md:text-4xl font-black tracking-tighter uppercase leading-[0.9] transition-all duration-300 mb-2 md:mb-3 italic drop-shadow-lg break-words lg:group-hover/card:[text-shadow:0_0_20px_#FFD700,0_0_40px_#FFD700]">
                        {legendaryPost.title}
                      </h3>
                      
                      <div className="grid transition-all duration-500 ease-in-out grid-rows-[1fr] lg:grid-rows-[0fr] lg:group-hover/card:grid-rows-[1fr]">
                        <div className="overflow-hidden opacity-100 lg:opacity-0 lg:group-hover/card:opacity-100 transition-opacity duration-500">
                    ...
                    <div className="hidden md:flex relative w-16 h-16 rounded-full border border-white/20 items-center justify-center text-white lg:group-hover/card:border-[#FFD700] lg:group-hover/card:text-[#FFD700] lg:group-hover/card:shadow-[0_0_20px_#FFD700] transition-all duration-700 flex-shrink-0 bg-pangea-neutral/20 backdrop-blur-md">
                      <BookOpen size={24} />
                    </div>
                 </div>
              </a>
