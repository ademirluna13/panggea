...
// 🔥 AQUÍ DECLARAMOS QUE RECIBE EL PROPS PARA QUE TYPESCRIPT NO LLORE 🔥
export default function Header({ searchData = [] }: { searchData?: any[] }) {
  ...
  const handleSearchChange = (value: string) => {
    ...
    if (value.length > 2) {
      setIsSearching(true);
      setTimeout(() => {
        // 🔥 FILTRAMOS DIRECTAMENTE DESDE EL PROP searchData 🔥
    ...
    <>
      ...
      `}>
        <header className={`relative transition-all duration-700 ease-[cubic-bezier(0.23,1,0.32,1)]
            ${isFloating 
              ? 'w-full max-w-7xl bg-pangea-card/95 backdrop-blur-2xl border border-pangea-primary/40 rounded-2xl h-16 px-6 md:px-10 shadow-pangea-neon' 
              : 'w-full bg-pangea-neutral md:bg-transparent border-b border-white/5 h-20 md:h-28 px-6 md:px-16'
            }`}
        >
          <div className="flex items-center justify-between h-full gap-4 xl:gap-8">
            ...
            <nav className="hidden lg:flex items-center gap-6 xl:gap-8">
              ...
              {menuGroups.map((group) => (
                ...
                >
                  ...
                  <div className={`absolute top-[calc(100%-10px)] left-1/2 -translate-x-1/2 w-56 bg-pangea-card/98 backdrop-blur-xl border border-pangea-primary/20 rounded-xl shadow-2xl py-4 transition-all duration-300 z-50
            ...
            {/* BÚSQUEDA FUNCIONAL */}
            <div className="flex-1 max-w-[180px] xl:max-w-md hidden lg:block relative" ref={searchRef}>
              ...
              {searchQuery.length > 2 && (
                <div className="absolute top-[calc(100%+5px)] right-0 w-[450px] bg-pangea-card/98 border border-pangea-primary/30 rounded-xl shadow-2xl overflow-hidden flex flex-col z-[150] backdrop-blur-3xl">
                  {isSearching ? (
          ...
          <div className="absolute bottom-0 left-0 w-full h-[2px] bg-white/5 overflow-hidden">
            <div className="h-full transition-all duration-200"
              style={{ width: `${scrollProgress}%`, background: `linear-gradient(to right, var(--pangea-primary), var(--pangea-secondary), var(--pangea-tertiary))` }}
            />
          </div>
        </header>
      </div>
      ...
      {/* MOBILE MENU */}
      <div className={`fixed inset-0 z-[9998] bg-pangea-neutral/98 backdrop-blur-3xl lg:hidden transition-all duration-500 flex flex-col ${isMenuOpen ? 'opacity-100 visible' : 'opacity-0 invisible pointer-events-none'}`}>
