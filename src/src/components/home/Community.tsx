...
export default function Community() {
  ...
  useGSAP(() => {
    // 1. MARQUEE INFINITO: La palabra PANGEA se mueve sola
    ...
    // 2. REVELADO DE CONTENIDO (Staggered)
    gsap.from(".comm-content > *", {
      scrollTrigger: {
        trigger: ".comm-content",
        start: "top 85%",
      },
      x: -50,
      opacity: 0,
    ...
    // 3. ENTRADA DE LA TARJETA (Zoom + Glow)
    gsap.from(".comm-card", {
      scrollTrigger: {
        trigger: ".comm-card",
        start: "top 80%",
  ...
  return (
    <section ref={container} className="relative bg-black py-32 overflow-hidden border-t border-white/5">
      ...
      {/* TEXTO GIGANTE DE FONDO (MARQUEE ANIMADO) */}
      <div className="absolute top-1/2 -translate-y-1/2 left-0 w-full pointer-events-none select-none overflow-hidden opacity-[0.03] whitespace-nowrap z-0">
        <div ref={marqueeRef} className="marquee-text inline-block">
          <span className="font-headline text-[25vw] font-black italic uppercase leading-none text-transparent px-20" style={{ WebkitTextStroke: '2px white' }}>
            PANGEA PANGEA PANGEA PANGEA
          </span>
          <span className="font-headline text-[25vw] font-black italic uppercase leading-none text-transparent px-20" style={{ WebkitTextStroke: '2px white' }}>
            PANGEA PANGEA PANGEA PANGEA
          </span>
      ...
      <div className="max-w-7xl mx-auto px-6 relative z-10">
        <div className="flex flex-col lg:flex-row items-center justify-between gap-16">
          ...
          {/* LADO IZQUIERDO: Contenido Animado */}
          <div className="comm-content max-w-2xl text-center lg:text-left">
            ...
            <h2 className="font-headline text-white text-6xl md:text-8xl font-[900] tracking-tighter leading-[0.85] uppercase mb-8">
              ÚNETE AL <br />
              <span className="text-pangea-primary">ECOSISTEMA</span>
            </h2>
          ...
          {/* LADO DERECHO: Tarjeta con Glow Volcánico */}
          <div className="comm-card w-full lg:max-w-md bg-[#0a0a0a]/80 p-8 md:p-12 border border-white/5 backdrop-blur-xl rounded-sm relative group overflow-hidden">
            ...
            <form className="relative z-10 flex flex-col gap-6" onSubmit={(e) => e.preventDefault()}>
              <div className="space-y-2 text-left">
                ...
                <input 
                  type="email" 
                  placeholder="USER@BITXOLO.APP"
                  className="w-full bg-black/50 border-b-2 border-white/10 p-4 text-white font-label text-sm focus:border-pangea-primary focus:ring-0 transition-all placeholder:text-white/10 uppercase tracking-widest"
                />
              </div>

              {/* BOTÓN ORO MAGMA (TERCIARIO) */}
              <button className="group relative w-full bg-pangea-tertiary text-black h-16 font-label text-xs font-[900] tracking-[0.3em] uppercase overflow-hidden transition-all hover:scale-[1.02] active:scale-95 shadow-[0_0_30px_rgba(255,215,0,0.2)] hover:shadow-[0_0_50px_rgba(255,215,0,0.4)] cursor-pointer">
